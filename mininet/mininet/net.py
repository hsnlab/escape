...
from term import cleanUpScreens, makeTerms
...
class Mininet( object ):
    ...
    def __init__( self, topo=None, switch=OVSKernelSwitch, host=Host,
                  ee=EE, agent=NetconfAgent, remoteSwitch=RemoteSwitch,
        ...
        """Create Mininet object.
           ...
           agent: default Agent class
           host: default Host class/constructor
           ee: default EE class/constructor
           remoteSwitch: default remoteSwitch class/constructor
           ...
           inNamespace: spawn switches and controller and agents in net namespaces?
        ...
        self.nameToNode = {}  # name to Node (Host/Switch/Agent) objects

        self.terms = []  # list of spawned xterm processes
        self.clickys = []  # list of spawned clicky processes
    ...
    def addRemoteSwitch( self, name, cls=None, **params ):
        """Add a remote switch.
        name: name of the switch to add
        cls: custom sw class/constructor
        params: optional parameters passed to the switch
        Returns: added switch"""
        defaults = {}
        defaults.update( params )
        if not cls:
            cls = self.remoteSwitch
        sw = cls( name, **defaults )
        self.switches.append( sw )
        self.nameToNode[ name ] = sw
        return sw

    def addManagedExe( self, name, nintf=2, **params):
        """Add an execution envirement which can host VNFs managed by
        an on site netconfd. This virtual node consists of an VNF
        container with nintf interfaces connected to a switch."""
        exe_switch = self.addSwitch(name + '_sw')
        exe_container = self.addEE(name + '_co')
        for intf in range(0, nintf):
            self.addLink( exe_container, exe_switch )
        return [ exe_switch,  exe_container ]

    def addSwitch( self, name, cls=None, **params ):
        """Add switch.  name: name of switch to add cls: custom switch
           class/constructor (optional) returns: added switch side
    ...
    def addAgent( self, name, cls=None, **params ):
        """Add agent.
           ...
           returns: added agent"""
        defaults = { 'inNamespace': self.inNamespace }
        defaults.update( params )
        if not cls:
            cls = self.agent
        agt = cls( name, **defaults )
        self.agents.append( agt )
        self.nameToNode[ name ] = agt
        return agt
    ...
    def __iter__( self ):
        ...
        for node in chain( self.hosts, self.ees,
                           self.switches, self.agents, self.controllers ):
            yield node.name

    def __len__( self ):
        "returns number of nodes in net"
        return ( len( self.hosts ) + len( self.ees ) + len( self.switches ) +
                 len( self.agents ) + len( self.controllers ) )
    ...
    def configEEs( self ):
        "Configure a set of execution envirements."
        for ee in self.ees:
            info( ee.name + ' ' )
            intf = ee.defaultIntf()
            if intf:
                ...
                # Don't configure nonexistent intf
                ee.configDefault( ip=None, mac=None )
            # You're low priority, dude!
            # BL: do we want to do this here or not?
            # May not make sense if we have CPU lmiting...
            # quietRun( 'renice +18 -p ' + repr( vnf.pid ) )
            # This may not be the right place to do this, but
            # it needs to be done somewhere.
            ee.cmd( 'ifconfig lo up' )
            ee.cmd( 'echo 0 > /proc/sys/net/ipv4/icmp_echo_ignore_broadcasts' )
            #ee.start()
        info( '\n' )
    ...
    def configAgents( self ):
        ...
        #     info( agt.name + ' ' )
        #     intf = agt.defaultIntf()
        #     if intf:
        #         agt.configDefault()
        #     else:
        #         # Don't configure nonexistent intf
        #         agt.configDefault( ip=None, mac=None )
        #     agt.cmd( 'ifconfig lo up' )
        # info( '\n' )
        ## TODO
        pass
    ...
    def buildFromTopo( self, topo=None ):
        ...
        if topo.ees():
            info( '\n*** Adding VNFs:\n' )
        for eeName in topo.ees():
            self.addEE( eeName, **topo.nodeInfo( eeName ) )
            info( eeName + ' ' )
        ...
        # if topo.agents():
        #     info( '\n*** Adding agents:\n' )
        # for agtName in topo.agents():
        #     self.addAgent( agtName, **topo.nodeInfo( agtName) )
        #     info( agtName + ' ' )
    ...
    def startTerms( self ):
        ...
        self.terms += makeTerms( self.ees, 'ee' )
    ...
    def stopClickys( self ):
        "Kill each clicky."
        for cl in self.clickys:
            os.kill( cl.pid, signal.SIGKILL )
        cleanUpScreens()
    ...
    def start( self ):
        "Start controller and switches and agents."
        if not self.built:
            self.build()
        ...
        if self.ees:
            info( '*** Setting up control network for EEs\n' )
...
class MininetWithControlNet( Mininet ):
    ...
    snum = 0 # To access the next address from outside
