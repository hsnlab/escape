...
def subtractNodeRes(current, substrahend, link_count = 1):
    ...
    if reduce(lambda a,b: a or b, (current[attr] == None for  attr in attrlist)):
        raise uet.BadInputException("Node resource components should always "+\
              "be given", "One of %s`s components is None"%str(current))
    if reduce(lambda a,b: a or b, (current[attr] < substrahend[attr] \
              for attr in attrlist if attr != 'bandwidth' and \
                                       substrahend[attr] is not  None)):
        return None
    ...
    try:
        for w in G:
            if G.node[w].type != 'SAP':
                for u in G:
                    for v in G:
                        if dist[u][v] > dist[u][w] + \
                                G.node[w].resources['delay'] + dist[w][v]:
                            dist[u][v] = dist[u][w] + \
                                G.node[w].resources['delay'] + dist[w][v]
    except KeyError as e:
...
def shortestPathsBasedOnEdgeWeight(G, source, target=None, cutoff=None):
    ...
    while fringe:
        ...
        for w, keydata in G[v].items():
            minweight, edgekey = min(((dd.weight, k)
                                      for k, dd in keydata.items()),
                                     key = lambda t: t[0])
            edata.append((w, edgekey, {'weight': minweight}))

        for w, ekey, edgedata in edata:
            vw_dist = dist[v] + getattr(G.node[w], 'weight', 0)\
                + edgedata['weight']
...
class MappingManager(object):
    ...
    def __init__(self, net, req, chains):
                    ...
                    for n,dn in net.network.nodes_iter(data=True):
                        if dn.type == 'SAP':
                            if dn.name == sapname:
                                ...
                                break
                    if not sapfound:
                        self.log.error("No SAP found in network with name: %s"
                                       %sapname)
                        raise uet.MappingException("No SAP found in network "+\
                  "with name: %s. SAPs are mapped exclusively by their names."%
                                                   sapname)
        ...
        self.chain_subchain.add_nodes_from((c['id'], 
                        {'avail_latency': c['delay']}) for c in chains)
    ...
    def addChain_SubChainDependency(self, subcid, chainids, subc, link_ids):
        ...
        self.chain_subchain.add_node(subcid, last_used_host = \
                            self.getIdOfChainEnd_fromNetwork(subc[0]),
                            subchain = zip(subc[:-1],subc[1:],link_ids))
        for cid in chainids:
            if cid not in {c['id'] for c in self.chains}:
                raise uet.InternalAlgorithmException("Invalid chain identifier"+\
                                                     "given to MappingManager!")
            else:
    ...
    def getLocalAllowedLatency(self, subchain_id, vnf1=None, vnf2=None, 
        ...
        if vnf1 is not  None and vnf2 is not  None and linkid is not  None:
            if self.req.network[vnf1][vnf2][linkid].type != 'SG':
                raise uet.InternalAlgorithmException("getLocalAllowedLatency "+\
                                  "function should only be called on SG links!")
        ...
        try:
            ...
            for c in self.chain_subchain.neighbors_iter(subchain_id):
                ...
                elif self.chain_subchain.node[c]['avail_latency'] < \
                        chain_maxlat:
                    chain_maxlat = \
                        self.chain_subchain.node[c]['avail_latency']

            return min(chain_maxlat, link_maxlat)

        except KeyError as e:
            raise uet.InternalAlgorithmException("Bad construction of chain-"%\
                                                     "subchain bipartie graph!")
    ...
    def isVNFMappingDistanceGood(self, vnf1, vnf2, n1, n2):
        ...
        for i,j,linkid,d in self.req.network.edges_iter([vnf1], 
                                                data=True, keys=True):
            if self.req.network[i][j][linkid].type != 'SG':
                self.log.warn("There is a not SG link left in the Service "+\
                                  "Graph, but now it didn`t cause a problem.")
                continue
            if j == vnf2:
                ...
                for c,chdata in self.chain_subchain.nodes_iter(data=True):
                    if 'subchain' in chdata.keys():
                        if (vnf1,vnf2,linkid) in chdata['subchain']:
                            ...
                            lal = self.getLocalAllowedLatency(c, vnf1, vnf2, 
                                                              linkid)
