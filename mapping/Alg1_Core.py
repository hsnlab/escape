...
class CoreAlgorithm(object):
    ...
    def __init__(self, net0, req0, chains0):
        ...
        self.log.setLevel(uet.loglevel)
        self.log.addHandler(uet.loghandler)
        ...
        self.pref_params = dict( cpu = dict(c=0.4,e=2.5), mem = \
         dict(c=0.4,e=2.5), storage = dict(c=0.4,e=2.5), 
    ...
    def _preproc(self, net0, req0, chains0):
        ...
        self.preprocessor = GraphPreprocessor.\
            GraphPreprocessorClass(net0, req0, chains0, self.manager)
        self.net = self.preprocessor.processNetwork( )
        self.req, self.chains_with_subgraphs = \
            self.preprocessor.processRequest( self.net )
    ...
    def _calculateAvgLinkUtil(self, path_to_map, linkids, bw_req, vnf_id = None):
        ...
        if vnf_id is not None:
            if self.req.node[vnf_id].resources['bandwidth'] is not None:
                ...
                avail_bw = self.net.node[path_to_map[-1]].availres['bandwidth']-\
                    internal_bw_req
                if avail_bw < 0:
        ...
        if len(path_to_map) > 1:
            ...
            if is_it_sap >= 0:
                ...
                return -1
            sum_w /= (len(path_to_map) - 1) + \
                (len(path_to_map) - sap_count_on_path)
                        
        elif len(path_to_map) == 0:
            self.log.warn("Tried to calculate average link utilization"\
                              +" on 0 length path!")
            return -1
            ...
            sum_w = float(max_bw - (act_bw - bw_req - internal_bw_req)) \
                / max_bw
    ...
    def _objectiveFunction(self, cid, node_id, prev_vnf_id, vnf_id, reqlinkid,
        ...
        if len(path_to_map) == 0:
            raise uet.InternalAlgorithmException("No path given to host %s for"\
                        "preference value calculation!"%node_id)

        if available['mem'] >= requested['mem'] and \
                available['cpu'] >= requested['cpu'] and \
                available['storage'] >= requested['storage']:
            avg_link_util = self._calculateAvgLinkUtil( path_to_map, linkids,
                                                        bw_req, vnf_id )
            if avg_link_util == -1:
            ...
            if sum_latency == -1 or sum_latency > local_latreq or not \
                    self.manager.isVNFMappingDistanceGood(prev_vnf_id, vnf_id, 
                    path_to_map[0], path_to_map[-1]):
                return -1, sys.float_info.max
            ...
            for attr, res_w in zip(['cpu', 'mem', 'storage'],
                ...
                float(maxres[attr]  - (available[attr] - requested[attr]))\
                    / maxres[attr], attr)
            ...
            scaled_bw_comp = 10 * float(self._preferenceValueOfUtilization(\
                                  avg_link_util, 'bandwidth')) / \
            ...
            self.log.debug("avglinkutil pref value: %f, sum res: %f, sumlat:%f"%\
                          (self.bw_factor * scaled_bw_comp, \
                           self.res_factor * scaled_res_comp, \
                           self.lat_factor * scaled_lat_comp))
            
            return self.bw_factor * scaled_bw_comp + \
                self.res_factor * scaled_res_comp + \
                self.lat_factor * scaled_lat_comp, sum_latency
        else: 
            return -1, sys.float_info.max
    ...
    def _updateGraphResources(self, bw_req, path, linkids, vnf=None, node=None):
        ...
        if vnf is not None and node is not None:
            if self.net.node[node].type != 'INFRA':
                raise uet.InternalAlgorithmException("updateGraphResources "+\
                          "should only be called on Infra nodes!")
            newres = helper.subtractNodeRes(self.net.node[node].availres, 
                             self.req.node[vnf].resources)
            if newres == None:
                raise uet.InternalAlgorithmException("During network resource "+\
                  "update, Infra node %s`s resources shouldn`t got below zero!"\
        ...
        if len(path) == 0:
            ...
            if self.net.node[ path[0] ].type != 'SAP':
                ...
                if new_bw <= 0:
                    self.net.node[ path[0] ].weight = \
                        sys.float_info.max # maybe use float("inf")?
                else:
    ...
    def _mapOneVNF(self, cid, subgraph, start, prev_vnf_id, vnf_id, reqlinkid):
        ...
        paths, linkids = helper.shortestPathsBasedOnEdgeWeight(subgraph, 
                                                                     start)
        for map_target in paths:
            if self.net.node[map_target].type == 'INFRA' and \
                    self.net.node[map_target].supported is not None:
                if self.req.node[vnf_id].functional_type in \
                        self.net.node[map_target].supported:
                    ...
                    if len(place_crit) > 0 and map_target not in place_crit:
                        ...
                        if value > -1 and value < best_node[1]:
                            ...
                            self.log.debug(\
                                "Calculated value: %f for VNF %s and path: %s"\
                                 %(value, vnf_id, paths[map_target]))

        if best_node[0] > -1:
            self.log.debug("Mapped VNF %s to node %s in network."\
                               %(vnf_id, best_node[0]))
            ...
            self.log.debug("Request Link %s, %s, %s mapped to path: %s"\
                               %(prev_vnf_id, vnf_id, reqlinkid, 
                                 paths[ best_node[0]]))
            self.manager.link_mapping.add_edge(prev_vnf_id, vnf_id, 
                                               key = reqlinkid,
            ...
            self._updateGraphResources(\
                ...
                paths[ best_node[0] ], linkids[ best_node[0] ], 
                vnf_id, best_node[0])
    ...
    def _mapOneRequestLink(self, cid, g, vnf1, vnf2, reqlinkid):
        ...
        if self._calculateAvgLinkUtil(path, linkids, bw_req) == -1:
            ...
            raise uet.MappingException("Last link of chain or best-effort link"\
                          " %s, %s, %s couldn`t be mapped due to link capacity"%\
                                           (vnf1, vnf2, reqlinkid))
        elif self.manager.getLocalAllowedLatency(cid, vnf1, vnf2, reqlinkid) < \
                self._sumLatencyOnPath(path, linkids):
            raise uet.MappingException("Last link %s, %s, %s of chain "\
                       "couldn`t be mapped due to latency requirement."\
                                           %(vnf1,vnf2,reqlinkid))
        self.log.debug("Last link of chain or best-effort link %s, %s "
                       %(vnf1, vnf2))
        ...
        self.manager.link_mapping.add_edge(vnf1, vnf2, key = reqlinkid,
                                           mapped_to = path,
                                           path_link_ids = linkids)


    def _addSAPandLinkToFromIt(self, netportid, bis_id, nffg, 
                               nodenf, reqportid, fc, toSAP=True):
        """
        ...
        if sapname_and_id not in nffg.network:
            sap = nffg.add_sap(name=sapname_and_id, 
                               id=sapname_and_id)
        else:
        ...
        if toSAP:
            nffg.add_sglink(nodenf.ports[reqportid], sap.add_port(), 
                            flowclass=fc)
        else:
            nffg.add_sglink(sap.add_port(), nodenf.ports[reqportid],
                            flowclass=fc)
    ...
    def _addFlowrulesToNFFGDerivatedFromReqLinks(self, v1, v2, reqlid, nffg):
        ...
        if len(path) == 1:
            ...
            if flowdst is None or flowsrc is None:
                raise uet.InternalAlgorithmException("No InfraPort found for "+\
                                    "a dynamic link of collocated VNFs")
            ...
            if nffg.network.node[path[0]].type != 'SAP':
                ...
                action_str = "output="
                if flowsrc == None:
                    raise uet.InternalAlgorithmException("No InfraPort "+\
                           "found for a dynamic link which starts a path")
                match_str += flowsrc.id
                if reqlink.flowclass != None:
                    match_str += ";flowclass=%s"%reqlink.flowclass
            ...
            if nffg.network.node[path[-1]].type != 'SAP':
                ...
                if flowdst == None:
                        raise uet.InternalAlgorithmException("No InfraPort "+\
                             "found for a dynamic link which finishes a path")
                action_str += flowdst.id + ";UNTAG"
                self.log.debug("Finishing flowrule %s => %s added to Port %s"\
                               " of %s"%(match_str,action_str,
                               self.net[path[-2]][path[-1]][linkids[-1]].dst.id,
                           path[-1]))
                nffg.network[path[-2]][path[-1]][linkids[-1]].dst.\
                    add_flowrule(match_str, action_str)
    ...
    def returnMappedNFFGofOneBiSBiS(self, bis_id):
        ...
        for vnf, host in self.manager.vnf_mapping:
            if bis_id == host:
                ...
                for i,j,k,d in self.req.out_edges_iter([vnf], 
                                                         data=True, keys=True):
                    # i is always vnf
                    path = self.manager.link_mapping[vnf][j][k]['mapped_to']
                    linkids = self.manager.link_mapping[vnf][j][k]\
                        ['path_link_ids']
                    if len(path) > 1:
                        # add a SAP with the name of the id of this BiS-BiS 
                        # concatenated the outbound port identifier.
                        self._addSAPandLinkToFromIt(\
                            self.net[path[0]][path[1]][linkids[0]].src.id, 
                            bis_id, nffg, nodenf, d.src.id, d.flowclass)
                    elif len(path) == 1:
                        # A collocation happened between vnf and j
                        nodenf_next = self._retrieveOrAddVNF(nffg, j)
                        nffg.add_sglink(nodenf.ports[d.src.id], 
                                        nodenf_next.ports[d.dst.id],
                                        flowclass=d.flowclass)
                    else:
                        raise uet.InternalAlgorithmException("No mapping given"+\
                         "for a link, after the algorithm has finished running!")
                # current vnf is already added to nffg
                for i,j,k,d in self.req.in_edges_iter([vnf],
                                                      data=True, keys=True):
                    # j is always vnf, link is i-->vnf directed
                    path = self.manager.link_mapping[i][vnf][k]['mapped_to']
                    linkids = self.manager.link_mapping[i][vnf][k]\
                        ['path_link_ids']
                    # collocation is handled by the out_edges_iter()
                    if len(path) > 1:
                        self._addSAPandLinkToFromIt(\
                            self.net[path[-2]][path[-1]][linkids[-1]].dst.id, 
                            bis_id, nffg, nodenf, d.dst.id, d.flowclass, 
                            toSAP=False)
            # TODO: construct EdgeReqs too, decide how E2E requirements should be
            # divided between sub NFFG-s
            # TODO: Transit INFRA nodes are not handled yet, only those which has
            ...
            if self.req.node[vnf].type == 'NF':
                ...
                for i,j,k,d in self.req.out_edges_iter([vnf], 
                    ...
                    nffg.add_undirected_link(out_infra_port, 
                                      mappednodenf.ports[d.src.id], dynamic=True)
                ...
                for i,j,k,d in self.req.in_edges_iter([vnf], 
                    ...
                    nffg.add_undirected_link(in_infra_port, 
                                      mappednodenf.ports[d.dst.id], dynamic=True)
            ...
            if self.req.node[i].type == 'SAP':
                ...
                if self.req.node[j].type == 'SAP':
                    ...
                    nffg.add_sglink(nffg.network.node[sapstartid].add_port(), 
                                    nffg.network.node[sapendid].add_port(), 
                                    id=d.id, flowclass=d.flowclass)
                else:
                    nffg.add_sglink(nffg.network.node[sapstartid].add_port(),
                                    nffg.network.node[j].add_port(),
                                    id=d.id, flowclass=d.flowclass)
    ...
    def start(self):
        for c, sub in self.chains_with_subgraphs:
            last_used_node = self.manager.\
                getIdOfChainEnd_fromNetwork(c['chain'][0])
            ...
            for curr_vnf, next_vnf, linkid in \
                    zip(c['chain'][:-1], c['chain'][1:], c['link_ids']):
                
                # Last element of chain is already mapped or SAP, if not 
                # mapped do it now!
                if self.req.node[next_vnf].type != 'SAP' and \
                        self.manager.getIdOfChainEnd_fromNetwork(next_vnf) == -1:
                    last_used_node = self._mapOneVNF(c['id'], sub, 
                                     last_used_node, curr_vnf, next_vnf, linkid)
                else:
                    ...
                    self._mapOneRequestLink(c['id'], sub, c['chain'][-2], 
                                            c['chain'][-1], c['link_ids'][-1])
...
def _main():
    try:
        net_graph, req_graph, chain_req = GraphInitializer.example1()
        alg = CoreAlgorithm ( net_graph, req_graph, chain_req )
        vnfs, links = alg.start()
        print "The VNF mappings are (vnf_id, node_id):", pformat(vnfs)
        print "\n The link mappings are:", pformat(links.edges(data=True))
    except uet.UnifyException as ue:
        print type(ue).__name__ + ": " + ue.msg, sys.exc_info()
