...
class GraphPreprocessorClass(object):
    ...
    def _findSubgraphForChain(self, chain):
        ...
        if sap_end == -1 or sap_begin == -1:
            raise uet.InternalAlgorithmException("Called subgraph finding with"+\
                                  " one of the chain ends not yet mapped!")
        
        current = sap_begin
        pending = []
        visited = set([sap_begin])
        dist = 0
        ...
        if sap_end not in visited:
            self.log.error("Chain end is further than latency requirement, "\
                               "for chain: %s"%chain)
            raise uet.MappingException("Chain end is further than latency"\
                                       " requirement, for chain: %s"%chain)
        ...
        self.log.debug("Subraph returned: %s" % subg.edges(keys=True))
        return subg
    ...
    def _colorLinksAndVNFs(self, e2e_w_graph, not_e2e):
        ...
        coloring and subgraph finding correctrly. '''
        ...
        for vnf in colored_req.nodes_iter():
            colored_req.node[vnf]['color'] = frozenset(
                colored_req.node[vnf]['color'])
        for i,j,k in colored_req.edges_iter( keys=True ):
            colored_req[i][j][k]['color'] = frozenset(
                colored_req[i][j][k]['color'])
        
        for i,j,k,d in colored_req.edges_iter( data=True, keys=True ):
            if len(d['color']) == 0:
                # TODO: proper handling of these links and not E2E chains 
                # not cointained by E2E chains.
                raise uet.BadInputException("All request edges should be "+\
                   "included in some E2E chains - at least in this version",
                   " Request link %s, %s, id: %s is not in any SAP-SAP chain"\
                                                %(i,j,k))
    ...
    def _getIntersectionOfSubgraphs_by_ChainIds(self, chain_ids, \
        ...
        for c,g in chains_w_graphs:
            if c['id'] in chain_ids:
                intersect.remove_nodes_from(n for n in intersect.nodes_iter()
                                            if n not in g)
    ...
    def _getNextIdOfSubchain(self, curr_vnf, act_color, colored_req, path):
        for curr,j,k,d in colored_req.out_edges_iter( \
                                     [curr_vnf], data = True, keys = True):
            if j in path:
                continue
    ...
    def _findOneSubchain(self, start, act_color, colored_req, e2e_w_graph):
        ...
        curr_vnf = start
        next_vnf, linkid = self._getNextIdOfSubchain(start, act_color, 
                                                     colored_req, subc_path)
        if next_vnf == None:
            self.log.error("Subchain with color %s cannot consist of only "\
                 "one VNF: %s"%(act_color, start))
            raise uet.InternalAlgorithmException("Subchain with color %s cannot "\
                           "consist of only one VNF: %s"%(act_color, start))
        while next_vnf is not  None:
            ...
            if not self.rechained[next_vnf]:
                ...
                if hash(colored_req.node[next_vnf]['color']) != hash(act_color):
                    subg_intersect = \
                       self._getIntersectionOfSubgraphs_by_ChainIds( \
                       colored_req.node[next_vnf]['color'], e2e_w_graph).nodes()
                    current_place_crit = \
                        self.req_graph.network.node[next_vnf].placement_criteria
                    if len(current_place_crit) > 0:
                        new_place_crit = \
                          [n for n in subg_intersect if n in current_place_crit]
                        if len(new_place_crit) == 0:
                            raise uet.MappingException("Given and internal" + \
                                 " placement criteria has no intersection.")
                        self.req_graph.network.node[next_vnf].placement_criteria\
                            = new_place_crit
                    else:
                        self.req_graph.network.node[next_vnf].placement_criteria\
        ...
        subg = self._getIntersectionOfSubgraphs_by_ChainIds(act_color, 
                                                            e2e_w_graph)
        ...
        self.log.debug("Subchain added: %s \n with subgraph: %s" %
                       (subc, subg.edges()))

        for i,j,k in zip(subc_path[:-1], subc_path[1:], link_ids):
            self.link_rechained.add_edge(i,j,key=k)

        self.manager.addChain_SubChainDependency(subc['id'], act_color, 
                                                 subc_path, link_ids)
    ...
    def _divideIntoDisjointSubchains(self, e2e_w_graph, not_e2e):
        ...
        e2e_sorted = sorted([t[0] for t in e2e_w_graph], 
                            key=lambda c: c['delay'])

        rechaining_cycles = 0
        while colored_req.number_of_edges() != 0:
            e2e = e2e_sorted[ rechaining_cycles % len(e2e_sorted) ]
            vnfs_sorted_by_degree = sorted([tup for tup in \
               colored_req.out_degree_iter() if tup[1] != 0], key=lambda t: t[1])
            ...
            for vnf, deg in vnfs_sorted_by_degree:
                ...
                # subchain
                act_color = frozenset([])
                for currnode,j,k,d in \
                        colored_req.out_edges_iter( [vnf], data=True, keys=True):
                    if e2e['id'] in d['color'] and \
                            not self.link_rechained.has_edge(vnf, j, key = k): 
                        act_color = d['color']
                        break
                    else:
                        continue
                ...
                if  len(act_color) > 0 and self.rechained[vnf]:
                    subc_subg = self._findOneSubchain(vnf, act_color, 
                                                      colored_req, e2e_w_graph)
                    ...
                    for i,j,k in zip(subc_subg[0]['chain'][:-1], 
                        ...
                        try:
                            ...
                            raise uet.InternalAlgorithmException("Mistake in"+\
                              " colored request graph maintenace, error: %s"%e)
                    break
        ...
        if not reduce(lambda a,b: a and b, self.rechained.values()): 
            ...
            raise uet.InternalAlgorithmException("There is a VNF, which is "\
                                                 "not in any subchain!!")
        if not reduce(lambda a,b: a and b, 
                      [self.link_rechained.has_edge(i,j, key=k ) 
                       for i,j,k in self.req_graph.network.edges_iter( \
                                                   keys=True )]):
            self.log.critical("There is a link, which is not in any subchain!!")
            raise uet.InternalAlgorithmException("There is a link, which is "\
                                                 "not in any subchain!!")
        ...
        # prioratized to being cut into less number of  peaces, so they are 
        # guaranteed to be mapped in bigger peaces, maybe still not enough reason
        starting_subchains = [c for c in output \
                  if self.req_graph.network.node[c[0]['chain'][0]].type ==\
                  'SAP']
        ...
        while len(output) != 0:
            next_subchains = [c for c in output if reduce(lambda a,b: a or b, 
                                [self._compareSubchainSubgraphTuples(d, c,
                                     use_latency=False) == -1\
                                     for d in sorted_output])]
            output = [c for c in output if c not in next_subchains]
            sorted_output.extend(sorted(next_subchains, 
                                 cmp = self._compareSubchainSubgraphTuples))
    ...
    def processRequest(self, preprocessed_network):
        ...
        for n in self.req_graph.infras:
            ...
            # host Infra node will reamin in the NFFG.
            self.req_graph.del_node(n.id)
        
        for i,j,k,d in self.req_graph.network.edges_iter(data=True, keys=True):
            if d.type != 'SG':
                raise uet.BadInputException("After removing the infras, only "+\
                      "the service graph should remain.", 
                      "Link %s between nodes %s and %s is a %s link"\
                                                %(k,i,j,d.type))
        ...
        for vnf, data in self.req_graph.network.nodes_iter(data=True):
            if data.type == 'SAP':
                ...
                raise uet.BadInputException("After preprocessing stage, only "+\
                      "SAPs or VNFs should be in the request graph.", 
                      "Node %s, type: %s is still in the graph"%(vnf,data.type))
        ...
        for vnf in self.req_graph.network.nodes_iter():
            if not hasattr(self.req_graph.network.node[vnf], 
                           'placement_criteria'):
                setattr(self.req_graph.network.node[vnf], 
                        'placement_criteria', [])
        ...
        for chain in self.chains:
            ...
            for i,j,k in zip(node_path[:-1], node_path[1:], link_ids):
                if hasattr(self.req_graph.network[i][j][k], 'bandwidth'):
                    self.req_graph.network[i][j][k].bandwidth += \
                        chain['bandwidth']
                else:
                    setattr(self.req_graph.network[i][j][k], 'bandwidth',
                        chain['bandwidth'])

            # Find seperate subgraph only for e2e chains from the upper layer.
        ...
        if len(e2e_chains_with_graphs) == 0:
            ...
            raise uet.BadInputException("Request with SAP-to-SAP chains",\
                      "Service chains do not contain any SAP-to-SAP chain")
    ...
    def processNetwork(self):    
        ...
        for n in net.infras:
            ...
            for vnf in net.running_nfs(n.id):
                ...
                # bandwidth capacity
                newres = helper.subtractNodeRes(\
                    net.network.node[n.id].availres, 
                    net.network.node[vnf.id].resources)
                if newres == None:
                    raise uet.BadInputException("Infra node`s resources are "+\
                              "expected to represent its maximal capabilities ",
                              "The NodeNF(s) running on Infra node %s, use(s)"+\
        ...
        for i,j,k in net.network.edges_iter(keys=True):
            if net.network[i][j][k].type != 'STATIC': 
                ...
                raise uet.BadInputException("After removing the NodeNFs from "+\
                  "the substrate NFFG, there shouldn`t be DYNAMIC, SG or "+\
                  "REQUIREMENT links left in the network.", 
                  "Link %s between nodes %s and %s is a %s link"\
                              %(k,i,j,net.network[i][j][k].type))
        for n in net.nfs:
            raise uet.BadInputException("NodeNF %s couldn`t be removed from "+\
                                   "the NFFG"%net.network.node[n].id,
                                   "This NodeNF probably isn`t mapped anyehere")

        
        self.shortest_paths = helper.shortestPathsInLatency( net.network )
        self.manager.addShortestRoutesInLatency ( self.shortest_paths )
        self.log.debug("Initial shortest_paths (in ms): %s" \
                           %pformat(self.shortest_paths))
        ...
        for n,d in net.network.nodes_iter(data=True):
            if d.type != 'SAP':
                ...
                self.log.debug("Weight for node %s: %f" % 
                               (n, net.network.node[n].weight))
