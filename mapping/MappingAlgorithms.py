...
def MAP(request, network):
                    ...
                    if i == sglink.src.node.id and j == sglink.dst.node.id:
                        if len(path) == 2:
                            ...
                            if req.delay is not  None:
                                setattr(request.network[i][j][sglink.id], 
                                        'delay', req.delay)
            ...
            if not is_there_notSGlink_in_path:
                if len(path) != 2:
                    if len(path) - 1 != len(chain['link_ids']):
                        raise Exception("TEMPORARY Exception: Not all "+\
                                        "link ID-s found for a service chain")
                    chain['id'] = cid
                    cid += 1
    ...
    for req in edgereqlist:
        ...
        mappedNFFG.add_req(srcnode.add_port(), dstnode.add_port(), 
