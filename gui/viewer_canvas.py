  ...
  def refresh (self):
    ...
    try:
      ...
      for e in self.canvas.dispG.edges():
        ...
        if not self.canvas.dataG.has_edge(u_dataG_id,
                                          v_dataG_id) and not \
           self.canvas.dataG.has_edge(
             v_dataG_id, u_dataG_id):
          # TODO needed to add switched edge to-from because a particular
          # link would've been removed. Need to investigate further
          for id in self.canvas.dispG.edge[e[0]][e[1]]:
            t_id = self.canvas.dispG.edge[e[0]][e[1]][id]['token_id']
            self.canvas.delete(t_id)
    ...
    for n in iterator:
      ...
      if not found:
        ...
        id = self.canvas.create_window(50, 50, window=t, anchor=tk.CENTER,
                                       tags='node')
        self.canvas.dispG.add_node(id,
                                   {'dataG_id': n, 'token': t, 'token_id': id})
    return
      ...
      if not self.canvas.dispG.has_edge(frm_disp, to_disp):
        ...
        for key, data in edges.items():
          ...
          if isinstance(self.canvas.dataG, nx.MultiGraph):
            ...
            dataG_id = (u, v)
          self.canvas.dispG.add_edge(frm_disp, to_disp, key,
                                     {'dataG_id': dataG_id,
                                      'dispG_frm': frm_disp,
                                      'token': token,
                                      'm': m})
          ...
          cfg = token.render()
          l = self.canvas.create_line(x1, y1, xa, ya, x2, y2, tags='edge',
                                      smooth=True, **cfg)
