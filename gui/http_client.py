...
class GetClient():
  ...
  def getNFFG(self, address = None):
    ...
    try:
      if not "http://" in self.__address[:7] and not "https://" in self.__address:
        self.__address = 'http://' + self.__address
      scheme, netloc, path, params, query, fragment= url.urlparse(self.__address)
      if netloc == '' or netloc == None:
        #assume localhost
        netloc = 'localhost'
