...
class PolicyEnforcementMetaClass(type):
    """
    Meta class for handling policy enforcement in the context of classes inhereted from AbstractVirtualizer

    If the PolicyEnforcement class contains a function which name matches one in the actual Virtualizer
    then PolicyEnforcement's function will be called first
    ...
    def __new__(mcs, name, bases, attrs):
        for attr_name, attr_value in attrs.iteritems():
            if isinstance(attr_value, types.FunctionType) and not attr_name.startswith('__'):
                # TODO - PolicyEnforcement is hard-coded. It should be do with loose coupling
                if hasattr(PolicyEnforcement, attr_name):
                    attrs[attr_name] = mcs.get_wrapper(attr_name, attr_value)

        return super(PolicyEnforcementMetaClass, mcs).__new__(mcs, name, bases, attrs)
