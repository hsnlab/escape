...
class NFIBManager(object):
  ...
  def initialize (self):
    ...
    # add new high-level NF to the DB, all the information related to the NF should be given as a dict 
    ...
    log.debug("%s : high-level  NFs were added to the DB" % self.__class__.__name__)
    
    # generate a  decomposition for a high-level forwarder NF (in form of networkx)
    G1 = networkx.DiGraph()
    ...
    # generate a decomposition for a high-level compressor NF (in form of networkx)
    G2 = networkx.DiGraph()
    ...
    # generate a decomposition for a high-level decompressor NF (in form of networkx)
    G3 = networkx.DiGraph()
    ...
    self.addClickNF({'label':'NF','node_id':'RFC2507Decomp','type':'click'})
    ...
    log.debug("%s : NF decompositions were added to the DB" % self.__class__.__name__)
